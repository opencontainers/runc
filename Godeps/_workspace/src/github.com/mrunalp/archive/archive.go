...
import (
	...
	"github.com/mrunalp/archive/pools"
)
...
var (
	...
	defaultArchiver   = &Archiver{Untar: Untar}
)
...
func DecompressStream(archive io.Reader) (io.ReadCloser, error) {
	...
	case Xz:
		...
		return newReadCloserWrapper(readBufWrapper, func() error {
			<-chdone
...
func TarWithOptions(srcPath string, options *TarOptions) (io.ReadCloser, error) {
	...
	patterns, patDirs, exceptions, err := cleanPatterns(options.ExcludePatterns)
		...
		for _, include := range options.IncludeFiles {
			...
			filepath.Walk(walkRoot, func(filePath string, f os.FileInfo, err error) error {
				...
				if include != relFilePath {
					skip, err = optimizedMatches(relFilePath, patterns, patDirs)
					if err != nil {
...
// optimizedMatches will assume that the inputs have been preprocessed and therefore the function
