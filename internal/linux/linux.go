...
func SetMempolicy(mode uint, flags []uint, nodes []int) error {
	modeWithFlags := mode
	...
	_, _, errno := unix.Syscall(unix.SYS_SET_MEMPOLICY, uintptr(modeWithFlags), uintptr(nodemaskPtr), uintptr(len(nodemask)*64))
	if errno != 0 {
