...
func (c *Container) killDeadInit(startTime uint64) {
	...
	var err error
	if p, ok := c.initProcess.(*initProcess); ok {
		// This is "runc run", init is our child and we need to reap it.
		err = ignoreTerminateErrors(p.terminate())
	} else {
		// This is "runc start", init is not our child: only kill it.
		// It will be reaped by e.g. host init, and so its cgroup may
		// briefly remain populated after we return.
		err = c.initProcess.signal(unix.SIGKILL)
	}
	if err != nil {
