...
func (c *linuxContainer) criuSwrk(process *Process, req *criurpc.CriuReq, opts *CriuOpts, applyCgroups bool) error {
	...
	for true {
		...
		t := resp.GetType()
		logrus.Debugf("RPC response from CRIU: %s", t)
		switch {
		...
		case t == criurpc.CriuReqType_PRE_DUMP:
			...
			logrus.Debugf("PRE_DUMP finished. Killing CRIU")
			err := cmd.Process.Kill()
			if err != nil {
				logrus.Debugf("After PRE_DUMP CRIU killing failed")
				return err
			}
			return nil
		default:
