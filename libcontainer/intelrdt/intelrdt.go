...
func NewManager(config *configs.Config, id string, path string) *Manager {
	// Quick check whether Intel RDT is available.
	// TODO: replace with Root() once PR 3306 is merged.
...
// is actually available. To be used from unit tests that mock intelrdt paths.
func newManager(config *configs.Config, id string, path string) *Manager {
	return &Manager{
		config: config,
		id:     id,
