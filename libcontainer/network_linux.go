...
func netnsAttach(name string, nsPath string, device configs.LinuxNetDevice) error {
	...
	// retain the alias if set
	if link.Attrs().Alias != "" {
		alias := nl.NewRtAttr(unix.IFLA_IFALIAS, []byte(link.Attrs().Alias))
		req.AddData(alias)
	}

	// set MTU if requested
	if device.MTU > 0 {
		mtu := nl.NewRtAttr(unix.IFLA_MTU, nl.Uint32Attr(uint32(device.MTU)))
		req.AddData(mtu)
	}
...
func netnsDettach(name string, nsPath string, device configs.LinuxNetDevice) error {
	...
	if nsLink.Attrs().Alias != "" {
		alias := nl.NewRtAttr(unix.IFLA_IFALIAS, []byte(nsLink.Attrs().Alias))
		req.AddData(alias)
	}
