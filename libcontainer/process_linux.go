...
func resetCPUAffinity(pid int) error {
	...
	if err := unix.SchedSetaffinity(pid, &cpuset); err != nil {
		return fmt.Errorf("reset cpu affinity: sched_setaffinity (all %d cpus): %w", cpuset.Count(), err)
	}
	return nil
}
...
func (p *initProcess) start() (retErr error) {
	...
	if err := resetCPUAffinity(p.pid()); err != nil {
		return err
	}
	if p.intelRdtManager != nil {
