...
func TestCreateNetDevices(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "multiple network devices and addresses",
			netDevices: map[string]specs.LinuxNetDevice{
				"eth1": {
					Addresses: []string{"192.168.1.2/24", "fd00:1:2::9/64"},
				},
				"eth2": {
					Name: "ctr_eth2",
				},
		...
		{
			name: "multiple network devices and hardware address",
			netDevices: map[string]specs.LinuxNetDevice{
				"eth1": {
					Addresses:       []string{"192.168.1.2/24", "fd00:1:2::9/64"},
					HardwareAddress: "e2:85:68:80:43:7a ",
				},
				"eth2": {
					Name: "ctr_eth2",
				},
		...
		{
			name: "multiple network devices and mtu",
			netDevices: map[string]specs.LinuxNetDevice{
				"eth1": {
					Addresses:       []string{"192.168.1.2/24", "fd00:1:2::9/64"},
					HardwareAddress: "e2:85:68:80:43:7a ",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for name, netdev := range config.NetDevices {
				...
				if !reflect.DeepEqual(ctrNetDev.Addresses, netdev.Addresses) {
					t.Fatalf("expected %v got %v", ctrNetDev.Addresses, netdev.Addresses)
				}
				if ctrNetDev.HardwareAddress != netdev.HardwareAddress {
					t.Fatalf("expected %s got %s", ctrNetDev.HardwareAddress, netdev.HardwareAddress)
				}
				if ctrNetDev.MTU != netdev.MTU {
					t.Fatalf("expected %d got %d", ctrNetDev.MTU, netdev.MTU)
				}
			}
		})
	}
